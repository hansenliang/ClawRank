...
export async function generateMetadata({ params }: { params: Promise<{ username: string; agentSlug: string }> }): Promise<Metadata> {
 const { username, agentSlug } = await params;
 if (!VALID_SLUG.test(username) || !VALID_SLUG.test(agentSlug)) return {};

 const detailSlug = `${username}/${agentSlug}`;
...
export default async function AgentDetailPage({ params }: { params: Promise<{ username: string; agentSlug: string }> }) {
 const { username, agentSlug } = await params;

 if (!VALID_SLUG.test(username) || !VALID_SLUG.test(agentSlug)) notFound();
 ...
 return (
  <main className="shell">
   <WindowChrome title={`clawrank://agent/${detailSlug}`}>
