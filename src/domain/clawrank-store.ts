...
export function getAgentDetail(store: ClawRankStore, slug: string, period: LeaderboardPeriod = 'alltime', now = new Date()): AgentDetail | null {
 ...
 methodologyNote:
 'Pilot path only: OpenClaw transcript data is translated into ClawRank-native daily agent facts, then queried from persisted ClawRank records. tyle reuse is fenced to adapter and validation patterns only.',
 generatedAt: now.toISOString(),
 };
}
